...
export default async function SecurityResultsByIdPage({
  params,
}: {
  ...
  const { data: assessment, error } = await (serviceRole.from('assessments') as any)
    .select('*')
    .eq('id', id)
    .eq('type', 'security')
    .single();
  ...
  const { score, risk_level: risk, compliant, partial, critical, gaps: allGaps } = row;
